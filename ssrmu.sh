...
Del_iptables(){
	if [[ ! -z "${port}" ]]; then
		iptables -D INPUT -m state --state NEW -m tcp -p tcp --dport 1443:1543 -j ACCEPT
		iptables -D INPUT -m state --state NEW -m udp -p udp --dport 1443:1543 -j ACCEPT
...
menu_status
  ...
  ${Green_font_prefix}---- HABLESSH ----${Font_color_suffix}
